...
## エクマン輸送の計算
地衡流に加えてエクマン流も熱を輸送する。エクマン流は海面風応力と粘性係数から計算する。海面風応力は人工衛星の観測値やそれを取り込んだ気象の再解析値を用いる。粘性係数は渦粘性係数であり、海洋表層での乱流状態によって決まる。渦粘性係数を精度良く推定することは（現状では）できないので、各深度でのエクマン流を推定することは諦めて、（渦粘性係数に依存しない、エクマン流を鉛直に積分した）エクマン輸送（ $T_x$ 、 $T_y$ [m $^2$ /s]）を考える。風応力（ $\tau_x$ 、 $\tau_y$ ）との関係は  
 $T_x=\tau_y/(\rho f), T_y=-\tau_x/(\rho f)$
で与えられる。エクマン輸送は混合層で生じていると考える。
...
流速が求まったら、海水の単位体積あたりの内部エネルギー（ $\rho C_p T$ [J/m $^3$] ）と流速（ $u,v$ [m/s]）の積を計算して、単位面積あたりの熱輸送量（単位は W/m $^2$ ）を計算する。流速の断面積を乗じれば、合計の熱輸送（単位は W=J/s ）が計算できる。エクマン輸送を用いた熱輸送の場合、混合層全体にエクマン流が均一に流れていると仮定して、混合層で平均した内部エネルギーとエクマン輸送の積を考えればよい。混合層の定義は、最上層の水温（密度）から0.2度（0.03 kg/m $3$ ）変化した深さまでとするのが一般的である。
