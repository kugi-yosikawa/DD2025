...
自然現象に含まれる様々な変動を成分に分け、そのうち主要な（卓越する）成分を抽出する統計手法。

## 考え方の基本

+ 空間構造パターン（モード）をもとに成分に分解する。
  + 分解された異なる成分は異なる空間構造を持つ。
  + 分解された各成分は、空間構造を保ったまま振幅変化する
+ 観測データはモードの重ね合わせ（線型結合）で表す。
+ 観測される変動強度（分散）への寄与率（モードの変動強度／全体の変動強度）に基づいてモードを順番づけして抽出する。
